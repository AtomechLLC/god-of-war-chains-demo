...
| 1-02-02 | 02 | 1 | VAL-01 | T-1-02 | human applies GUI config; ini persisted | checkpoint + artifact | `test -s "$USERPROFILE/Documents/PCSX2/inis/PCSX2.ini"` | ❌ created by human session | ⬜ pending |
...
| 1-03-01 | 03 | 2 | VAL-01 | — | N/A | checkpoint + artifact | `ls "$USERPROFILE/Documents/PCSX2/snaps/"*.png >/dev/null 2>&1` | ❌ created by human session | ⬜ pending |
| 1-03-02 | 03 | 2 | VAL-01 | T-1-01 | media stays in gitignored dirs | artifact | `test -f reference/frames/calib.png && [ "$(ls reference/frames | grep -Ec '_f[0-9]+_.+_(close|mid|wide)\.png')" -ge 9 ]` | ❌ created by task | ⬜ pending |
